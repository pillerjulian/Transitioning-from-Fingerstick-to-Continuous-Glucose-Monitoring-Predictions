...
def masked_mse(y_true, y_pred):
    # Create a mask where finger measurements are missing (assuming 0 denotes missing)
    mask = K.cast(K.not_equal(y_true, 0), K.floatx())
    # Calculate MSE only for masked entries
    return K.mean(K.square(mask * (y_true - y_pred)), axis=-1)
...
def train_model(x_train, y_train, batch_size, epochs, learning_rate):
    ...
    model = Sequential()
    model.add(LSTM(units=50, return_sequences=True, input_shape=(x_train.shape[1], num_features)))
    model.add(LSTM(units=50, return_sequences=True))  # Keep return_sequences=True
    model.add(Dropout(0.5))
    model.add(TimeDistributed(Dense(1)))  # Predicting 'cbg' value at each timestep

    custom_optimizer = Adam(learning_rate=learning_rate)
    model.compile(optimizer=custom_optimizer, loss=masked_mse)

    history = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1)

    return model, history

def fill_missing_predictions(finger_data, predictions):
    # Assuming 0 in finger_data indicates a missing value
    mask = (finger_data == 0)
    # Only fill in the missing (masked) values with predictions
    finger_data[mask] = predictions[mask]
    return finger_data

def make_prediction(scalers_transforms_test, model, x_test, finger_data):
    # Make predictions
    ...
    # Here, we're assuming that finger_data is the original array with missing values indicated by 0
    filled_predictions = fill_missing_predictions(finger_data, predictions)
    ...
    # Now, calculate RMSE only for the originally missing data
    mask = (finger_data == 0)
    rmse = np.sqrt(np.mean(((filled_predictions[mask] - finger_data[mask]) ** 2)))

    return predictions, continuous_predictions, rmse

# Example usage:
# finger_data = np.array([1, 0, 0, 0, 5, 0, 0, 0, 9])
# predictions should be the same shape as finger_data and contain the model's predictions
# predictions = model.predict(some_input)

# filled_finger_data = fill_missing_predictions(finger_data, predictions)
