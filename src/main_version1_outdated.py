...
def read_patient(train_set, test_set, finger_window, prediction_window):
    ...
    ground_truth_cgm = [] # list with not scaled blood glucose from y_test not broken into arrays
    ...
    while (i >= prediction_window and i < len(scaled_test_set)):
        ...
        for bg in test_set[i-prediction_window:i,0]:
            ground_truth_cgm.append(bg) # creates a 1D ground truth vector of cgm, used for plotting only
    ...
    return x_train, y_train, x_test, y_test, ground_truth_cgm, scalers_transforms_train, scalers_transforms_test, smbg_scatter
...
def make_prediction(scalers_transforms_test, model, x_test, y_test):
    ...
    predicted_cgm = predictions[0] # 1D array with the predicted cgm values, used for plotting only

    for i in range(1,len(predictions)):
        predicted_cgm = np.concatenate([predicted_cgm,predictions[i]])
    ...
    return predictions, predicted_cgm, rmse
